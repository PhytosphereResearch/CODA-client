...
const EditAgents = (props) => {
  ...
  const onAgentSelected = (option) => {
    ...
    getAgent(option.value).then((agent) => setSelectedAgent(agent));
  };
  ...
  const onSynonymChange = (e) => {
    ...
    const alreadyExists =
      newAgent && agents.find((agent) => isLikelyRepeat(agent, synonym));
  ...
  const onSubmit = async () => {
    ...
    update({ agent, accessToken, userName }).then(() => resetState());
  };
  ...
  return (
    <div>
      ...
      <p>
        If the agent does not yet exist in CODA, add it by filling in the
        information below. Depending on the agent, some fields will not be
        applicable:
      </p>
      {newAgent ? (
        <div style={{ display: 'flex', gap: '8px' }}>
          ...
          <TextInput
            title="Species"
            value={selectedSynonym.species}
            name="species"
            onChange={onSynonymChange}
          />
          <TextInput
            title="Sub-species"
            value={selectedSynonym.subSpecies}
            name="subSpecies"
            onChange={onSynonymChange}
          />
          <TextInput
            title="Taxonomic authority"
            value={selectedSynonym.authority}
            name="authority"
            onChange={onSynonymChange}
          />
        </div>
      ...
      {fields.data ? (
        <>
          ...
          <RadioGroup
            title="Ecology"
            selected={selectedAgent.ecology}
            name="ecology"
            options={ECOLOGY}
            onChange={onInputChange}
          />
          <TextInput
            title="Common Name"
            value={selectedAgent.commonName}
            name="commonName"
            onChange={onInputChange}
          />
          <TextArea
          ...
          />
          <RadioGroup
            title="In field guide"
            selected={selectedAgent.mostCommon}
            name="mostCommon"
            options={BOOLEANS}
            onChange={onInputChange}
          />
          ...
          <TextArea
            ...
            value={selectedAgent.originalCodaRecord}
            name="originalCodaRecord"
          />
        </>
      ) : null}
