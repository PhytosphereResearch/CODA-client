...
const EditSynonyms = (props) => {
  ...
  const onAgentSelected = (option) => {
    ...
    getAgent(option.value).then((agent) => {
      let selectedSynonym = agent.synonyms.find(
        (synonym) => synonym.id === option.synId,
      );
  ...
  const createSynonym = (event) => {
    if (
      (event.target.value === EDIT && !newSynonym) ||
      (event.target.value === ADD && newSynonym)
    ) {
      return;
  ...
  const submitSynonym = async () => {
    ...
    update({ synonym: submittedSynonym, accessToken, userName }).then(
      resetState(),
    );
  };
  ...
  const otherSynonyms =
    selectedAgent && selectedAgent.otherSynonyms.length ? (
      <div>
